...
public class ChooseUnitActivity extends BaseActivity implements View.OnClickListener,BusinessResponse {
    ...
    String crop = "";
	int currentItem = -1;
	String type_code = "PRICEUNIT_TYPE";
...
//		type_code = getIntent().getStringExtra("type_code");
		...
		dictionaryModel.getUnit(AppConst.info_from,type_code);
	...
	private void updateData() {
		if (dictionaryModel.data.unit.size() > 0) {
			if (cropAdapter == null) {
	...
	public void OnMessageResponse(String url, JSONObject jo, AjaxStatus status) throws JSONException {
		...
		if (url.contains(ApiInterface.UNIT)){
			if (dictionaryModel.data.unit.size() > 0)
			{
				crop_type = dictionaryModel.data.unit;
				updateData();
			}
