...
public class PublishFindFriendActivity extends BaseActivity implements OnClickListener, BusinessResponse, OnAddressSelectedListener
{
	GridView gridView;
	...
	private static final int REQUEST_FIND_HELPER = 3;
...
//		setContentView(R.layout.e01_public_sell);
		setContentView(R.layout.publish_find_friend);
		...
		gridView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				...
				if ("000000".equals(imgs) ){
					PhotoPickerIntent intent = new PhotoPickerIntent(PublishFindFriendActivity.this);
					...
					PhotoPreviewIntent intent = new PhotoPreviewIntent(PublishFindFriendActivity.this);
	...
	public void onClick(View v) {
		...
		switch(v.getId()) {
			...
			case R.id.text_position:
				positionDialog = new BottomDialog(PublishFindFriendActivity.this);
				positionDialog.setOnAddressSelectedListener(PublishFindFriendActivity.this);
				positionDialog.show();
				break;
			case R.id.text_type:
...
//				showDialog("选择类别",datalist);
				...
				if (findhelper_type == 0)
					intent.putExtra("type_code","ZHAOBANGSHOU_CODE");
				else
					intent.putExtra("type_code","WOSHIBANGSHOU_CODE");
				startActivityForResult(intent,REQUEST_FIND_HELPER);
...
//				startActivityForResult(intent,REQUEST_UNIT);
	...
	private void submit() {
		...
		if (TextUtils.isEmpty(subtitle))
			...
			request.source = "1";
			publishModel.publishFindHelper(request);
		}
	}
	...
	private void showDialog(String title, final ArrayList<String> datalist) {
		AlertDialog.Builder builder = new AlertDialog.Builder(PublishFindFriendActivity.this,AlertDialog.THEME_HOLO_LIGHT);
		View view = LayoutInflater.from(PublishFindFriendActivity.this).inflate(R.layout.dialog,null);
		builder.setView(view);
		((TextView)view.findViewById(R.id.text_title)).setText(title);
		ListView listview = (ListView) view.findViewById(R.id.listview);
		DialogAdapter adapter = new DialogAdapter(PublishFindFriendActivity.this,datalist);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (data != null) {
			if (requestCode == REQUEST_FIND_HELPER) {
	...
	public void OnMessageResponse(String url, JSONObject jo, AjaxStatus status)
		...
		if (url.contains(ApiInterface.PUBLISHFINDHELPER))
		{
	...
	private class GridAdapter extends BaseAdapter {
		...
		public GridAdapter(ArrayList<String> listUrls) {
			...
			inflater = LayoutInflater.from(PublishFindFriendActivity.this);
		}
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (path.equals("000000")){
				...
				Glide.with(PublishFindFriendActivity.this)
						.load(path)
