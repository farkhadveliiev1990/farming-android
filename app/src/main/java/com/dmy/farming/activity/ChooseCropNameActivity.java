...
public class ChooseCropNameActivity extends BaseActivity implements View.OnClickListener,BusinessResponse {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.choose_crop_name);
		...
		dictionaryModel.cropsubType(AppConst.info_from,crop_code,true);
		...
		switch (v.getId()) {
			...
			case R.id.text_finish:
				if (currentItem >= 0) {
					...
					sub_crop_code = crop_type.get(currentItem).code;
				}
				Intent intent = new Intent();
				intent.putExtra("crop",crop);
				intent.putExtra("crop_code",sub_crop_code);
	...
	public void OnMessageResponse(String url, JSONObject jo, AjaxStatus status) throws JSONException {
		...
		if (url.contains(ApiInterface.CROPSUB)){
			if (dictionaryModel.data.crop_sub_type.size() > 0)
			{
				crop_type = dictionaryModel.data.crop_sub_type;
				updateData();
			}
