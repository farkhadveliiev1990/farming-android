...
public class PopupList {
    ...
    public void init(Context context, View anchorView, List<String> popupItemList, OnPopupListClickListener onPopupListClickListener) {
        ...
        if (mScreenWidth == 0) {
            mScreenWidth = AppUtils.getScWidth(mContext);
        }
        if (mScreenHeight == 0) {
            mScreenHeight = AppUtils.getScHeight(mContext);
        }
...
//        if (mContextView instanceof ChatRow) {
//            View bubble = ((ChatRow)mContextView).getBubbleLayout();
//
//            int[] locRow = new  int[2] ;
//            int[] locBubble = new  int[2] ;
//
//            mContextView.getLocationInWindow(locRow);
//            bubble.getLocationInWindow(locBubble);
//
//            Log.e("y", mContextView.getClass() + " " + mContextView.getTop() + " " + mContextView.getBottom() + " " + locRow[1] + " " + locBubble[1]);
//            mPopupWindow.showAtLocation(mAnchorView, Gravity.CENTER,
//                    locBubble[0] + bubble.getWidth() / 2  - mScreenWidth / 2,
//                    mContextView.getTop() + (locBubble[1] - locRow[1]) + mPopupWindowHeight + dp2px(5) + - mScreenHeight / 2);
//
//        }
